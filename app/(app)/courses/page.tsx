...
export default async function CoursesPage() {
  ...
  const { data: courses } = await supabase
    .from("courses")
    .select("id, name, city, country, par_total, hole_count, tees")
      ...
      {!courses || courses.length === 0 ? (
        <Card className="p-10 text-center">
          ...
          <p className="mt-2 text-muted">
            Add the tracks you play — tees, par, slope, rating, hole yardages and stroke index.
          </p>
          <div className="mt-6">
            <Link href="/courses/new">
              ...
              <Link key={c.id} href={`/courses/${c.id}`}>
                <Card className="card-hover p-5 h-full">
                  ...
                  <div className="mt-5 grid grid-cols-3 gap-3 text-center">
                    ...
                    <Metric
                      label={primary ? `Slope · ${primary.label}` : "Slope"}
                      value={primary ? primary.slope : "—"}
                    />
                    <Metric
                      label="Rating"
                      value={primary ? primary.rating.toFixed(1) : "—"}
                    />
                  </div>
                  {tees.length > 1 && (
                    <div className="mt-3 flex flex-wrap gap-1">
                      {tees.map((t) => (
                        <span
                          key={t.label}
                          className="text-[10px] uppercase tracking-wider rounded-full border border-border px-2 py-0.5 text-muted"
                        >
                          {t.label}
                        </span>
                      ))}
